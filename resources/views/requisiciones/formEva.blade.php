                            ...
                            @if ($format->format == $i)
                                <div class="@if ($i == 5) col-span-2 @endif">
                                    <div class="mb-4  flex justify-start items-center gap-4">
                                        ...
                                        @elseif($noEvaluation != 1 && $format->valid)
                                            <input name="anexo{{ $i }}" value="{{ $format->valid }}"
                                                class="reviewCheckbox form-check-input" type="checkbox"
                                                id="check-review-{{ $format->id }}">
                                        @endif
                                        <label class="form-check-label" for="check-review2-{{ $format->id }}">
                                            ...
                                            @if ($format->valid == false && $format->justification)
                                                <span class="text-red-600">({{ $format->justification }})</span>
                                            @elseif ($format->valid == true)
                                                <span class="text-green-400">&#10003; {{ $format->justification }}</span>
                                            @endif
                                        </label>
                                    </div>
                                    @if ($noEvaluation != 1 && $format->valid)
                                        <div>
